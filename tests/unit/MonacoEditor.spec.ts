...
describe('MonacoEditor.vue', () => {
  it('should mount editor', async () => {
    ...
    expect(monaco.editor.create.mock.calls.length).toBe(1);
  });
  it('should pass all props', async () => {
    resetEditorMock();
    const props = {
      value: 'v1',
      language: 'l1',
    ...
    await meInstance.vm.$nextTick();
    expect(monaco.editor.create.mock.calls[0][1]).toEqual(expectedEditorOptions);
  });
  it('prop updates should be reflected in editor', async () => {
    const editorInstance = resetEditorMock();
    const props = {
      value: 'v1',
      language: 'l1',
  ...
  it('editor code update should trigger input event', async () => {
    const editorInstance = resetEditorMock();
